...
A notable engineering decision is that the application uses a **manual composition root** in `main.dart`: although `get_it` and `injectable` are configured, the entire dependency graph (secure storage, the shared Dio client, interceptors, repositories, use cases, and BLoCs) is wired by hand at startup. This was required to resolve a deliberate forward-reference: the token-refresh interceptor must be able to call back into the `AuthCubit` to force a sign-out on session expiry, but the interceptor is constructed before the cubit. A single shared `Dio` client and a single shared secure-storage session object are then reused across all authenticated features, so a refreshed token propagates everywhere at once.
...
A **local-first** list of blocked IP addresses — added either manually by the user or automatically by the detection pipeline. Every mutation is written to the local cache first (and enforced immediately at the network level through the VPN service), then mirrored best-effort to the backend `/blacklist` endpoints, so the list follows the user across devices without ever depending on connectivity. Removing an entry also lifts the block natively so connectivity is restored. Auto-blocks raised by the ML pipeline appear in the list in real time via a broadcast change stream, and the navigation bar shows a live blocked-count badge.
...
- **`BorderRadiusManager`**, **`FontManager`** / `text_style_manager`, and **`DecorationManager`** — centralise radii, typography (the bundled _Figtree_ family), and every `BoxDecoration`, gradient, shadow, and input decoration, including theme-dependent factories.
